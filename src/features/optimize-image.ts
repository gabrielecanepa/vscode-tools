...
const launch = async (app: string, files: vscode.Uri[]): Promise<void> => {
...
const resolveFile = (arg: unknown): vscode.Uri | undefined => {
...
const basename = (uri: vscode.Uri): string => uri.path.slice(uri.path.lastIndexOf("/") + 1);

const noAppConfigured = (): void => {
  void vscode.window.showErrorMessage(
    'Optimize Image: set an app name in the "vscodeTools.optimizeImages.app" setting first.',
  );
};

const optimizeImage = async (arg: unknown): Promise<void> => {
